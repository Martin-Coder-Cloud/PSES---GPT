...
"- When `demographic_breakdown_present=true` and `allow_trend=true`, and when at least two survey cycles contain valid values for at least two demographic groups, you MUST evaluate how the demographic gap has evolved over time (for example, remained identical, widened, or narrowed) and report this using allowable integer differences. In this situation, you must NOT write “There is no trend data available for prior years” for the demographic portion.\n"
