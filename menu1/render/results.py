...
from typing import Dict, Callable, Any, Tuple, List, Set
import io
import json
import hashlib
import re
...
def _pick_display_metric(df: pd.DataFrame, prefer: str | None = None) -> str | None:
    """
...
def _allowed_numbers_from_disp(df: pd.DataFrame, metric_col: str) -> tuple[Set[int], Set[int]]:
    ...
    for g, gdf in work.dropna(subset=["__Val__", "__Year__"]).groupby("Demographic", dropna=False):
        vals = list(int(v) for v in gdf.sort_values("__Year__")["__Val__"].tolist() if v is not None)
        # all pairwise absolute diffs across visible years
