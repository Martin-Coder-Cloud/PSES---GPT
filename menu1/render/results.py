...
def _meaning_labels_for_question(
    *,
    ...
    """
    Return list of aggregated option labels strictly from metadata (literal):
      • Read indices from Survey Questions row using reporting_field (positive/negative/agree)
      • Map indices to labels from Survey Scales where code matches question code
        (fallback: use 'scale' key from Survey Questions if present)
      • If indices missing: as a last resort, parse labels from metric_label of the form
        '% selecting A / B'; otherwise return []
    No canonical fallbacks, no inferred scales, no reordering or normalization.
    """
    try:
        ...
        # Determine column name from reporting_field; default to positive if unknown
        colname = (reporting_field or "").lower()
        if colname not in ("positive", "negative", "agree"):
            colname = "positive"

        # Get indices like "1,2"
        ...
        def _map_labels(code_key: str) -> List[str]:
            sc = meta_scales[meta_scales["code"] == str(code_key).strip().upper()]
            if sc.empty:
                return []
            m = {int(v): str(l) for v, l in zip(sc["value"], sc["label"])}
            return [m[i] for i in idxs if i in m]

        labels = _map_labels(qU)
        if not labels:
            # Try scale key if present
            for sk in ("scale", "scale_id", "scale_name"):
                if sk in row.columns:
                    key = row.iloc[0].get(sk, None)
                    if pd.notna(key) and str(key).strip():
                        labels = _map_labels(str(key))
                        if labels:
                            break
        return labels
    except Exception:
